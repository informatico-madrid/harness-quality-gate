...
```bash
python3 ${CLAUDE_SKILL_DIR}/harness_quality_gate.adapters.python.weak_test {project-root}/tests/ {project-root}/src/ 2>&1
```
...
```bash
python3 ${CLAUDE_SKILL_DIR}/harness_quality_gate.bmad.mutation_analyzer {project-root}/ 2>&1
```
...
```bash
python3 ${CLAUDE_SKILL_DIR}/Deferred {project-root}/tests/ 2>&1
```
