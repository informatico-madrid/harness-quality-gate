...
# ==================================================================
# bootstrap.ensure_venv  — mutmut_6, 8, 9, 29, 30, 34, 35, 39, 40, 44
# ==================================================================

class TestEnsureVenvSubprocessSpies:
    """Spy subprocess.run called by ensure_venv — verify exact args."""

    def test_ensure_venv_subprocess_run_exact_command_and_kwargs(self, tmp_path: Path):
        """Venv creation calls subprocess.run with exact cmd and kwarg values."""
        from harness_quality_gate.bootstrap import ensure_venv
        ...
        with patch("subprocess.run", spy_run):
        ...
        assert kwargs["check"] is True          # kills True→False, None
        assert kwargs["capture_output"] is True  # kills False, None
        assert kwargs["timeout"] == 60           # kills 59, 61, None
        assert "text" not in kwargs              # text not passed → killed if added


    def test_ensure_venv_no_call_when_exists(self, tmp_path: Path):
        """Existing venv skips subprocess.run entirely."""
        from harness_quality_gate.bootstrap import ensure_venv
        ...
        with patch("subprocess.run", counting_spy):
            ensure_venv(tmp_path)

        assert call_count[0] == 0, "ensure_venv must not call subprocess when venv exists"


    def test_ensure_venv_permission_error_passes_through(self, tmp_path: Path):
        """PermissionError from subprocess.run propagates as RuntimeError."""
        from harness_quality_gate.bootstrap import ensure_venv

        def throwing_spy(cmd, **kwargs):
            raise PermissionError("denied")

        with patch("subprocess.run", throwing_spy):
            with pytest.raises(RuntimeError, match="Cannot create venv"):
                ensure_venv(tmp_path)


    def test_ensure_venv_timeout_expired_returns_venv_dir(self, tmp_path: Path):
        """TimeoutExpired returns venv_dir without raising."""
        from harness_quality_gate.bootstrap import ensure_venv

        def timeout_spy(*args, **kwargs):
            raise subprocess.TimeoutExpired(cmd=args[0], timeout=60)

        with patch("subprocess.run", timeout_spy):
            result = ensure_venv(tmp_path)

        assert result == tmp_path / ".venv"

    def test_ensure_venv_timeout_logs_exact_warning_message(self, tmp_path: Path, caplog):
        """Mata: "venv creation timed out" → "XXvenv creation timed outXX" (§H3).

        The XX-wrap on the log format key must be observable by checking
        getMessage() returns the EXACT expected format + interpolated args.
        """
        import logging
        ...
        caplog.set_level(
            logging.WARNING, logger="harness_quality_gate.bootstrap"
        )

        def timeout_spy(*args, **kwargs):
            raise subprocess.TimeoutExpired(cmd=[sys.executable, "-m", "venv"], timeout=60)
        ...
        # exact string match — XX-wrap breaks the format string key entirely
        warning_msgs = [m for m in messages if m.startswith("venv creation timed out")]
        assert len(warning_msgs) >= 1
        # venv_dir path must be in the message (first %s placeholder)
        assert str(venv_path) in warning_msgs[0]
